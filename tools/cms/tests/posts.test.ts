...
async function setup(t:any,raw='---\ntitle: Original\npubDate: 2026-09-08\n---\n\n# Keep this\n\n[ref]: /image.png\n') {
  ...
  await fs.writeFile(path.join(root,'original.md'),raw);
...
test('opening and saving minimal frontmatter preserves bytes and mtime without adding defaults',async t=>{
  ...
  const file=path.join(root,'original.md'); const before=(await fs.stat(file)).mtimeMs;
...
test('metadata edits patch only changed values, preserving BOM, CRLF, comments, timestamps, quoting and body',async t=>{
  ...
  assert.equal(await fs.readFile(path.join(root,'original.md'),'utf8'),raw.replace("'Original'",'"Changed"'));
});
test('editing body preserves the exact frontmatter, while category persists when explicitly edited',async t=>{
  ...
  assert.equal(await fs.readFile(path.join(root,'original.md'),'utf8'),raw.slice(0,raw.indexOf('\n\n#'))+'\nNew body\n');
...
test('block scalar and block list changes preserve unrelated fields',async t=>{
  ...
  assert.match(await fs.readFile(path.join(root,'original.md'),'utf8'),/custom:  true # unchanged/);
});
test('duplicate preserves original, body and unknown fields, creates draft and rejects collisions/stale revisions',async t=>{
  ...
  assert.equal(await fs.readFile(path.join(root,'original.md'),'utf8'),raw);
  assert.match(await fs.readFile(path.join(root,'original-copy.md'),'utf8'),/custom:   \[a, b\] # untouched/);
...
test('rename is byte-preserving, does not overwrite collisions, and rejects unsafe paths and stale revisions',async t=>{
  ...
  await fs.writeFile(path.join(root,'taken.md'),'existing unrelated content');
  await assert.rejects(store.rename(p.id,'taken',p.revision),/already exists/);
  assert.equal(await fs.readFile(path.join(root,'taken.md'),'utf8'),'existing unrelated content');
  ...
  assert.equal(renamed.revision,p.revision);assert.equal(await fs.readFile(path.join(root,'new-name.md'),'utf8'),raw);
